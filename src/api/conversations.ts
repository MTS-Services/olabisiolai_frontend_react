...
export async function getConversations(params: {
  ...
  const res = await api.get<ApiResponse<Record<string, unknown>[]>>(`/conversations`, {
    params: {
      page: params.page ?? 1,
...
export async function getConversation(uuid: string): Promise<Conversation> {
  const res = await api.get<ApiResponse<Record<string, unknown>>>(`/conversations/${uuid}`)
...
export async function searchConversations(query: string): Promise<Conversation[]> {
  const res = await api.get<ApiResponse<Record<string, unknown>[]>>(`/conversations/search`, {
    params: { q: query },
  })
  const { data } = unwrapApi(res.data)
